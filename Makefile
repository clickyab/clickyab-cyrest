...
export LINTER=$(BIN)/gometalinter -e ".*.gen.go" --cyclo-over=20 --line-length=200 --deadline=100s --disable-all --enable=structcheck --enable=deadcode --enable=gocyclo --enable=ineffassign --enable=golint --enable=goimports --enable=errcheck --enable=varcheck --enable=interfacer --enable=misspell --enable=lll --enable=megacheck
