...
func selectorType(pkg humanize.Package, i *humanize.SelectorType) (swaggerType, error) {
	im, err := pkg.FindImport(i.Package)
	if err != nil {
		return nil, err
	}
	...
	for param, res := range transferList {
		...
		if i.Type.(*humanize.IdentType).Ident == ident[1] && im.Path == ident[0] {
			return res, nil
		}
	}

	newPkg, err := humanize.ParsePackage(im.Path)
	if err != nil {
		return nil, err
	}
...
func sliceToRml(pkg humanize.Package, arr *humanize.ArrayType) (swaggerType, error) {
	...
	case *humanize.SelectorType:
		i, err := pkg.FindImport(t.Package)
		if err != nil {
			return nil, err
		}
...
func init() {
	workDir := filepath.Join(os.Getenv("WORK_DIR"), "swagger")
	assert.Nil(os.MkdirAll(workDir, 0744))
	plugins.Register(&swaggerGenerator{workDir: workDir})
}
