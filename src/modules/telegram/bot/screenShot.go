...
func (bb *bot) uploadSS(bot *tgbotapi.BotAPI, m *tgbotapi.Message) {
	...
	for i := range channels {
		*channelNames = append(*channelNames, channels[i].Name)
	}
...
func init() {
	...
	tgbot.RegisterConversation(tgbot.Screenshot, tgbot.Chatter{
		...
		Handler: func(m *tgbotapi.Message, data map[int]interface{}) (string, bool) {
			...
			logrus.Warn(fp)
			assert.Nil(err)
			hhh, err := fila.UploadFromURL(fp, channel.UserID)
			logrus.Warn(hhh)

			bundleChannelAd := ads.NewAdsManager().FindBundleChannelAd(channel.ID, Fbundle.ID)
			bundleChannelAd.Shot = common.NullString{Valid: true, String: hhh}
